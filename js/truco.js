...
// ---- Multi-celular: cada grupo juega en una "sala" de Firebase; cada
// celular elige un asiento (Jugador 1..N) y ve el mismo estado en vivo. ----

let trucoRoomActual = null;
let misAsiento = null;
let trucoListenerEstado = null;
let trucoListenerAsientos = null;
let asientosActuales = {};

function trucoRoomId(modo){
  return modo === 'equipos' ? 'truco-grupo' : 'truco-pareja';
}

function trucoRefEstado(roomId){ return db.ref(`salas/${roomId}/estado`); }
function trucoRefAsientos(roomId){ return db.ref(`salas/${roomId}/asientos`); }

function trucoDesconectar(){
  if(trucoRoomActual){
    if(trucoListenerEstado) trucoRefEstado(trucoRoomActual).off('value', trucoListenerEstado);
    if(trucoListenerAsientos) trucoRefAsientos(trucoRoomActual).off('value', trucoListenerAsientos);
  }
  trucoListenerEstado = null;
  trucoListenerAsientos = null;
}

function trucoGuardar(){
  if(!trucoRoomActual) return;
  trucoRefEstado(trucoRoomActual).set(truco);
}

function trucoEntrarSala(modo, roomId){
  trucoListenerAsientos = trucoRefAsientos(roomId).on('value', snap => {
    asientosActuales = snap.val() || {};
    if(truco) renderTruco();
  });
  trucoListenerEstado = trucoRefEstado(roomId).on('value', snap => {
    const data = snap.val();
    if(!data){
...
function trucoElegirAsiento(modo, roomId, i){
  trucoRefAsientos(roomId).child(i).set(idDispositivo()).then(() => {
    misAsiento = i;
    localStorage.setItem('truco-asiento-' + roomId, i);
    trucoEntrarSala(modo, roomId);
  });
}

function trucoRenderEleccionAsiento(modo, roomId){
  const container = document.getElementById('truco-content');
  if(!container) return;
  const nombres = modo === 'equipos'
    ? ['Jugador 1', 'Jugador 2', 'Jugador 3', 'Jugador 4']
    : ['Jugador 1', 'Jugador 2'];
  container.innerHTML = `
    <div class="hero">
      <h2>¿Quién sos vos?</h2>
      <p>Elegí tu lugar en la mesa. Cada celular elige uno distinto.</p>
    </div>
    <div class="apuestas-row" style="flex-direction:column;">
      ${nombres.map((n, i) => {
        const ocupado = asientosActuales[i] && asientosActuales[i] !== idDispositivo();
        return ocupado
          ? `<button class="btn-apuesta secondary" disabled style="opacity:.5;cursor:not-allowed;">${n} (ocupado)</button>`
          : `<button class="btn-apuesta" onclick="trucoElegirAsiento('${modo}', '${roomId}', ${i})">${n}</button>`;
      }).join('')}
    </div>`;
}

function iniciarTruco(modo){
  const modoFinal = modo || (truco ? truco.modo : 'dos');
  const roomId = trucoRoomId(modoFinal);

  if(trucoRoomActual === roomId && misAsiento !== null){
    if(truco) renderTruco();
    return;
  }

  trucoDesconectar();
  trucoRoomActual = roomId;
  const guardado = localStorage.getItem('truco-asiento-' + roomId);
  misAsiento = guardado === null ? null : Number(guardado);

  trucoRefAsientos(roomId).once('value').then(snap => {
    asientosActuales = snap.val() || {};
    if(misAsiento !== null && asientosActuales[misAsiento] && asientosActuales[misAsiento] !== idDispositivo()){
      misAsiento = null;
      localStorage.removeItem('truco-asiento-' + roomId);
    }
    if(misAsiento === null){
      trucoRenderEleccionAsiento(modoFinal, roomId);
    } else {
      trucoEntrarSala(modoFinal, roomId);
    }
  });
}
...
function trucoNuevaMano(){
  ...
  const liderInicial = liderAnterior === null ? 0 : (liderAnterior + 1) % numJugadores;
...
function trucoJugarCarta(indiceCarta){
  if(truco.turno !== misAsiento) return;
  ...
  truco.cartasBazaActual = truco.cartasBazaActual || {};
  truco.cartasBazaActual[jugador] = carta;

  const todosJugaron = Object.keys(truco.cartasBazaActual).length === truco.numJugadores;
  if(todosJugaron){
    trucoResolverBaza();
  } else {
...
function trucoResolverBaza(){
  const cba = truco.cartasBazaActual || {};
  const jugadas = [];
  for(let i = 0; i < truco.numJugadores; i++){
    if(cba[i]) jugadas.push({ jugador: i, poder: poderCarta(cba[i]) });
  }
  ...
  truco.resultadosBazas = truco.resultadosBazas || [];
...
function trucoSubirTruco(){
  ...
  if(misAsiento !== p.respondePor) return;
...
function trucoResponder(quiero){
  if(misAsiento !== truco.pendiente.respondePor) return;
...
function trucoBadgeAsientoHTML(){
  if(misAsiento === null || !truco) return '';
  return `<div class="section-label">Vos sos ${truco.jugadores[misAsiento]}</div>`;
}
  ...
  for(let i = 0; i < truco.numJugadores; i++){
    slots += `
      <div class="truco-mesa-slot">
        ...
        ${cba[i] ? cartaHTML(cba[i]) : cartaDorsoHTML()}
      </div>`;
  }
  return `
...
function renderTruco(){
  ...
  if(truco.fase === 'jugando'){
    const jugador = truco.turno;
    const esperando = jugador !== misAsiento;
    const misCartas = misAsiento !== null ? (truco.cartasEnMano[misAsiento] || []) : [];
    container.innerHTML = `
      ${trucoBadgeAsientoHTML()}
      ${trucoMarcadorHTML()}
      ${trucoMesaHTML()}
      <div class="section-label">Tus cartas</div>
      <div class="truco-mano">
        ${misCartas.map((c, i) => `<div ${esperando ? '' : `onclick="trucoJugarCarta(${i})"`} style="${esperando ? 'opacity:.5;' : ''}">${cartaHTML(c)}</div>`).join('')}
      </div>
      ${esperando ? `<div class="hero" style="margin-top:8px;"><p>Esperando a ${truco.jugadores[jugador]}...</p></div>` : trucoApuestasRowHTML()}`;
    return;
  }
  ...
  if(truco.fase === 'respondiendo'){
    ...
    const puedoResponder = misAsiento === p.respondePor;
    let botonesSubir = '';
    if(puedoResponder){
    ...
    container.innerHTML = `
      ...
      <div class="hero" style="margin-top:8px;">
        ...
        <p>${puedoResponder ? `¿Qué decís?` : `Esperando la respuesta de ${truco.jugadores[p.respondePor]}...`}</p>
      </div>
      ${puedoResponder ? `<div class="apuestas-row">
